...
df['sma5'] = df['C'].rolling(window=3).mean()

df['std'] = df['C'].rolling(window=3).std()

df.dropna(inplace=True)

df['UB'] = df['sma5'] + df['std'] 
df['LB'] = df['sma5'] - df['std'] 
	...
	for item in orders[:]:
		if item.order_type == 'long':
			if todays_high > item.order_level:
				padding = (row.UB-row.LB)/8

				tmp_trade = trade(item.order_level, row.UB+8*padding, row.LB - padding, trade_id, 'long', row.Index, position_size)
			...
			if todays_low < item.order_level:
				...
				padding = (row.UB-row.LB)/8
				tmp_trade = trade(item.order_level, row.LB-8*padding, row.UB + padding, trade_id, 'short', row.Index, position_size)
				log_trade(trade_id, dates[row.Index], 'short', item.order_level, equity, 1/position_size)
				portfolio.append(tmp_trade)
				orders.remove(item)
				trade_id += 1

	# Check if we have the price action we are looking for, in that case we place order
	if todays_close < row.LB and yesterdays_close > row.LB:

		order = order_long_short(row.LB, 1, 'long')
		orders.append(order)
	elif todays_close > row.UB and yesterdays_close < row.UB:
